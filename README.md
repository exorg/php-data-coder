...
The common way to intall Composer is downloading installer via curl tool and call php interpreter to run it. 
...
```
If it's project set-up stage and no one dependencie have been installed yet run
```bash
composer install
```

If another dependencies have been intalled preciously, run
```bash
composer update
```
...
The simplest way to autoload all needed files in executable file is attach *autoload.php* file generated by Composer (after running `composer install` or `composer update` command) like in following example
...
This project has unit tests, which has been build with [PHPUnit](https://phpunit.de/) framework and run on Linux operating system.
